...
export async function createServerTestDatabase(): Promise<TestDatabase> {
	...
	try {
		...
		db.run(
			"CREATE TABLE agents (id TEXT PRIMARY KEY, title TEXT NOT NULL, description TEXT NOT NULL, logo TEXT NOT NULL, runtime TEXT NOT NULL, model TEXT NOT NULL, concurrency INTEGER NOT NULL, owner TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, skills TEXT NOT NULL, recent_work TEXT NOT NULL, activity TEXT NOT NULL, instructions TEXT NOT NULL)",
		);
		db.run(
...
export function seedServerTestDatabase(dbPath: string): void {
	...
	try {
		...
		db.run(
			"INSERT INTO agents (id, title, description, logo, runtime, model, concurrency, owner, created_at, updated_at, skills, recent_work, activity, instructions) VALUES ('agent-1', 'Codex Main', 'Primary implementation agent', 'bot', 'codex', 'gpt-5', 1, 'owner-1', '2026-05-12T00:02:00.000Z', '2026-05-12T00:02:00.000Z', '[\"typescript\"]', '[\"ROY-229\"]', '[\"created profile\"]', 'Follow project workflow instructions.')",
		);
		db.run(
